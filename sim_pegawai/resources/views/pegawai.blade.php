...
@section('css')
    <style>
        ...
        .avatar-box { background: linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%); color: #2563eb; width: 42px; height: 42px; border-radius: 12px; display: flex; justify-content: center; align-items: center; font-size: 1.2rem; box-shadow: 0 2px 5px rgba(37,99,235,0.15); border: 2px solid #ffffff; }
    ...
    <div class="container-fluid mt-3">
        <div class="page-header-custom d-flex justify-content-between align-items-center flex-wrap">
            <div class="mb-2 mb-md-0">
                <h1 class="m-0"><i class="fas fa-hospital-user mr-2"></i> Manajemen Pegawai</h1>
                <p class="mb-0 mt-1"><i class="fas fa-info-circle mr-1"></i> Pusat kendali data staf, perawat, dan administrasi klinik terintegrasi.</p>
            </div>
            <div>
                <button class="btn btn-tambah-header" data-toggle="modal" data-target="#modalTambah">
                    <i class="fas fa-plus-circle mr-1"></i> Tambah Data Pegawai
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="card card-custom">
        <div class="card-header card-header-custom d-flex justify-content-between align-items-center">
            <h3 class="card-title font-weight-bold" style="color: #0f172a; font-size: 1.1rem;">
                <i class="fas fa-list-ul mr-2 text-primary"></i> Daftar Seluruh Pegawai
            </h3>
            <div class="card-tools">
                <div class="input-group input-group-sm shadow-sm" style="width: 250px; border-radius: 20px;">
                    <div class="input-group-prepend">
                        <span class="input-group-text bg-white" style="border-radius: 20px 0 0 20px; border-right: none; border-color: #e2e8f0;"><i class="fas fa-search text-muted"></i></span>
                    </div>
                    <input type="text" name="search" class="form-control bg-white" placeholder="Cari nama pegawai..." style="border-radius: 0 20px 20px 0; border-left: none; border-color: #e2e8f0; box-shadow: none;">
                </div>
            </div>
        </div>
                    ...
                    <tbody>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="avatar-box mr-3">
                                        {{ substr($p->nama_pegawai, 0, 1) }}
                                    </div>
                                    <div>
                                        <div class="font-weight-bold" style="color: #0f172a; font-size: 1.05rem;">{{ $p->nama_pegawai }}</div>
                                        <div style="color: #64748b; font-size: 0.85rem;"><i class="fas fa-phone-alt mr-1" style="font-size: 10px;"></i> {{ $p->no_hp }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div style="color: #334155; font-weight: 700;">{{ $p->jabatan }}</div>
                                <div style="color: #94a3b8; font-size: 0.8rem;"><i class="fas fa-envelope mr-1"></i> {{ $p->email ?? 'Tidak ada email' }}</div>
                            </td>
                            <td class="text-center">
                                @if($p->status == 'aktif')
                                    <span class="badge-modern bg-soft-success"><i class="fas fa-check-circle mr-1"></i> Aktif Bekerja</span>
                                @else
                                    <span class="badge-modern bg-soft-danger"><i class="fas fa-ban mr-1"></i> Tidak Aktif</span>
                                @endif
                            </td>
                            <td class="text-center">
                                ...
                                <div class="modal-content">
                                    <form action="/pegawai/{{ $p->id }}" method="POST">
                                        ...
                                        <div class="modal-header bg-white">
                                            <h5 class="modal-title font-weight-bold" style="color: #0f172a;"><i class="fas fa-user-edit text-warning mr-2"></i> Ubah Data Pegawai</h5>
                                        ...
                                        <div class="modal-body bg-white">
                                            <div class="form-group mb-4">
                                                <label class="text-muted font-weight-bold" style="font-size: 0.8rem; letter-spacing: 0.5px;">NAMA LENGKAP <span class="text-danger">*</span></label>
                                                <div class="input-group">
                                                    <div class="input-group-prepend"><span class="input-group-text"><i class="fas fa-user"></i></span></div>
                                                    <input type="text" name="nama_pegawai" class="form-control font-weight-bold" value="{{ $p->nama_pegawai }}" required>
                                                </div>
                                            </div>
                                            <div class="form-group mb-4">
                                                <label class="text-muted font-weight-bold" style="font-size: 0.8rem; letter-spacing: 0.5px;">JABATAN <span class="text-danger">*</span></label>
                                                <div class="input-group">
                                                    <div class="input-group-prepend"><span class="input-group-text"><i class="fas fa-briefcase"></i></span></div>
                                                    <select name="jabatan" class="custom-select font-weight-bold" required>
                                                        <option value="Perawat" {{ $p->jabatan == 'Perawat' ? 'selected' : '' }}>Perawat</option>
                                                        <option value="Apoteker" {{ $p->jabatan == 'Apoteker' ? 'selected' : '' }}>Apoteker</option>
                                                        <option value="Bidan" {{ $p->jabatan == 'Bidan' ? 'selected' : '' }}>Bidan</option>
                                                        <option value="Petugas Pendaftaran" {{ $p->jabatan == 'Petugas Pendaftaran' ? 'selected' : '' }}>Petugas Pendaftaran</option>
                                                        <option value="Kasir" {{ $p->jabatan == 'Kasir' ? 'selected' : '' }}>Kasir</option>
                                                        <option value="Staf Rekam Medis" {{ $p->jabatan == 'Staf Rekam Medis' ? 'selected' : '' }}>Staf Rekam Medis</option>
                                                        <option value="IT Support" {{ $p->jabatan == 'IT Support' ? 'selected' : '' }}>IT Support</option>
                                                        <option value="Administrasi / HRD" {{ $p->jabatan == 'Administrasi / HRD' ? 'selected' : '' }}>Administrasi / HRD</option>
                                                        <option value="Cleaning Service" {{ $p->jabatan == 'Cleaning Service' ? 'selected' : '' }}>Cleaning Service</option>
                                                        <option value="Security" {{ $p->jabatan == 'Security' ? 'selected' : '' }}>Security</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 form-group mb-4">
                                                    <label class="text-muted font-weight-bold" style="font-size: 0.8rem; letter-spacing: 0.5px;">STATUS PEGAWAI</label>
                                                    <div class="input-group">
                                            ...
                                            <div class="form-group mb-0">
                                                <label class="text-muted font-weight-bold" style="font-size: 0.8rem; letter-spacing: 0.5px;">ALAMAT DOMISILI</label>
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-5">
                                <div style="color: #94a3b8;"><i class="fas fa-folder-open fa-4x mb-3"></i></div>
                                <h5 class="text-muted font-weight-bold">Tidak ada data pegawai</h5>
                                <p class="text-muted mb-0" style="font-size: 0.95rem;">Klik tombol "Tambah Data Pegawai" di atas untuk memulai.</p>
                            </td>
                        </tr>
                        @endif
                    </tbody>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="/pegawai" method="POST">
                @csrf
                <div class="modal-header bg-white">
                    <h5 class="modal-title font-weight-bold" style="color: #0f172a;"><i class="fas fa-user-plus text-primary mr-2"></i> Tambah Pegawai Baru</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body bg-white">
                    ...
                    <div class="form-group mb-4">
                        <label class="text-muted font-weight-bold" style="font-size: 0.8rem; letter-spacing: 0.5px;">JABATAN PEGAWAI <span class="text-danger">*</span></label>
                        <div class="input-group">
                            <div class="input-group-prepend"><span class="input-group-text"><i class="fas fa-briefcase"></i></span></div>
                            <select name="jabatan" class="custom-select" required>
                                ...
                                <option value="Staf Rekam Medis">Staf Rekam Medis</option>
                                <option value="IT Support">IT Support</option>
                                <option value="Administrasi / HRD">Administrasi / HRD</option>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 form-group mb-4">
                            <label class="text-muted font-weight-bold" style="font-size: 0.8rem; letter-spacing: 0.5px;">EMAIL AKSES</label>
                            <div class="input-group">
                    ...
                    <div class="form-group mb-0">
                        <label class="text-muted font-weight-bold" style="font-size: 0.8rem; letter-spacing: 0.5px;">ALAMAT DOMISILI</label>
                        <textarea name="alamat" class="form-control" rows="2" placeholder="Masukkan alamat lengkap (Opsional)"></textarea>
                    </div>
                </div>
                <div class="modal-footer bg-light" style="border-bottom-left-radius: 20px; border-bottom-right-radius: 20px; padding: 1.2rem 1.5rem;">
                    <button type="button" class="btn btn-light border font-weight-bold text-muted mr-2" data-dismiss="modal">Batalkan</button>
